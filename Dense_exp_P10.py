...
Instead of sparse rewards, the agent gets immediate feedback about coverage improvement.
...
Results saved in: ./alt_experiment_3_TIMESTAMP/
"""
...
class TrainingCallback(BaseCallback):
    ...
    def _on_step(self):
        if self.n_calls % self.eval_freq == 0:
            ...
            if self.verbose > 0:
                print(f"Step {self.num_timesteps}: Reward = {total_reward:.2f}, Dice = {dice_score:.3f}")
...
def run_experiment_3():
    ...
    results_folder = f"./experiment_3_{current_time}"
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_folder, 'training_progress.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    create_projections = True  # Set to True if you want vis
